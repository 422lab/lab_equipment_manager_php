...
$request = $data['request'];                // 接收$request请求号;Number;3位数字;

switch ($request) {
case 100:   // 设备端同步请求
    $status = $data['status'];              // 接收$status设备状态;String;"on":已上机,"off":未上机;
    $mac_addr = $data['mac'];               // 接收$mac设备MAC地址;String;"30:ae:a4:56:75:50";
    $arr = array(
        'request' => 100,                   // 发送$request请求号;Number;将接收到的请求号原样送回;
        ...
        'timer_time' => "15:00:00"          // 发送$timer_time预约时间;String;"HH:MM:SS",24小时制,已上机且有更新时发送;
    );
...
case 101:   // 设备端下机通知
    ...
    $mac_addr = $data['mac'];               // 接收$mac设备MAC地址;String;"30:ae:a4:56:75:50";
    $arr = array(
        'request' => 101                    // 发送$request请求号;Number;将接收到的请求号原样送回;
    );
    header('content-type:application/json');
    echo json_encode($arr);
    break;
